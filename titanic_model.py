...
all_d['Title'] = all_d['Title'].replace(['Lady','Countess','Capt','Col','Don','Dr','Major','Rev','Sir','Jonkheer','Dona'],'Rare')
...
all_d['Deck']          = all_d['Cabin'].str[0].map({'E':5,'D':4,'A':3,'B':2,'F':2,'C':1,'G':1,'T':0}).fillna(0).astype(int)
...
print(f"Train: {model.score(X, y):.4f}")

preds = model.predict(X_test)
print(f"Survivors: {preds.sum()} / 418")
