"""
Model v4 - Companion Survival Rate (BEST MODEL)
Leaderboard score: 0.78229
Key insight: If a passenger's ticket companions are in the training set,
their survival rate is a powerful predictor.
  - Companions all survived -> 70.9% chance you survived
  - Companions all died    -> 22.5% chance you survived
  - 152 test passengers have training companions (36% of test set)
This is computed with leave-one-out to avoid data leakage in training.
"""
...
# companion survival rate — leave-one-out for train to avoid leakage
...
pd.DataFrame({'PassengerId': test_ids, 'Survived': preds}).to_csv(
    os.path.join(base, 'submission_v4.csv'), index=False)
print("Saved: submission_v4.csv  <-- use this for best leaderboard score")
